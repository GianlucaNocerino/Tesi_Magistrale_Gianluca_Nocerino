...
L'unico file scritto su disco sono i grafici dei fit, in EXAMPLES_DIR.
 
Uso:
    python examples/OEWtoMTOW.py
...
EXAMPLES_DIR = Path("examples")   # unica cartella scritta: solo i grafici dei fit
...
def main() -> None:
    ...
    for tipo, gruppo in df.groupby("Type"):
        ...
        grafico_fit(m, EXAMPLES_DIR)
        modelli.append(m)
 
    if modelli:
        grafico_confronto(modelli, EXAMPLES_DIR)
