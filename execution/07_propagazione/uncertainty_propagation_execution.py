...
(prodotto da execution/06_incertezza_calibrazione/calibration_uncertainty_execution.py).

Uso (fai PRIMA un giro con N_SAMPLES piccolo!):
    python execution/07_propagazione/uncertainty_propagation_execution.py
...
execution/08_mappa_probabilistica/plot_probabilistic_technology_map.py.
...
# Configurazione
# ---------------------------------------------------------------------
OUT_DIR = Path(__file__).resolve().parent          # tutti gli output stanno qui
THETA_ACC_PATH = OUT_DIR.parent / "06_incertezza_calibrazione" / "theta_acc.csv"   # output della Fase 6
N_SAMPLES = 1500                        # <-- 200 per provare, 1000+ per la tesi
SEED = 0
N_WORKERS = 1                          # >1 richiede il guard __main__ (vedi sotto)
CHECKPOINT_DIR = OUT_DIR / "propagation_checkpoints"
CHUNK_SIZE = 25
MIN_CELLS = 3                          # celle di contatto minime per considerare
                                       # due tecnologie adiacenti sulla mappa
OUTPUT_PATH = OUT_DIR / "propagation_result.npz"
